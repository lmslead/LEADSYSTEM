...
const testConversionRate = async () => {
  try {
    ...
    const immediateEnrollmentLeads = await Lead.countDocuments({ callDisposition: 'Immediate Enrollment' });

    console.log('Current Statistics:');
    console.log(`Total Leads: ${totalLeads}`);
    console.log(`Qualified Leads: ${qualifiedLeads}`);
    console.log(`Immediate Enrollment Leads: ${immediateEnrollmentLeads}`);
    ...
    if (qualifiedLeads > 0 && immediateEnrollmentLeads === 0) {
      ...
      // Recalculate
      const newImmediateEnrollmentLeads = await Lead.countDocuments({ callDisposition: 'Immediate Enrollment' });
